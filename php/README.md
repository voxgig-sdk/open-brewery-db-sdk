# OpenBreweryDb PHP SDK



The PHP SDK for the OpenBreweryDb API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Brewery()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($brewerys as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["address_1"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $brewery));
```
...
```php
$client = OpenBreweryDbSDK::test($testopts, $sdkopts);
```
...
```
php/
├── openbrewerydb_sdk.php          -- Main SDK class
...
The main class (`openbrewerydb_sdk.php`) exports the SDK class

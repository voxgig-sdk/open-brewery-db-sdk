...
// OpenBreweryDb SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenBreweryDbOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenBreweryDbOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenBreweryDbOmni::structfix($val, Struct::undef()))) {

...
class BreweryEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = OpenBreweryDbSDK::test($seed, null);
        $seen = iterator_to_array($base->Brewery(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = OpenBreweryDbConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = OpenBreweryDbSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Brewery(null)->stream("list", null, null) as $item) {

...
// OpenBreweryDb SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OpenBreweryDbTestFeature extends OpenBreweryDbBaseFeature
{
    ...
    public function init(OpenBreweryDbContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OpenBreweryDbContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OpenBreweryDbParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OpenBreweryDbContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OpenBreweryDbContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OpenBreweryDbParam::call($ctx, $k);

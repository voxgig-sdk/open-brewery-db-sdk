...
class OpenBreweryDbSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = OpenBreweryDbHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = OpenBreweryDbHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!OpenBreweryDbFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, OpenBreweryDbFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return OpenBreweryDbUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = OpenBreweryDbHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = OpenBreweryDbHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = OpenBreweryDbHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new OpenBreweryDbError($op . "_allow",
                "OpenBreweryDbSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = OpenBreweryDbHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new OpenBreweryDbError("graphql_error",
                "OpenBreweryDbSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Brewery($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_brewery === null) {
                $this->_brewery = new BreweryEntity($this, null);
            }
            return $this->_brewery;
        }
        return new BreweryEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new OpenBreweryDbSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}

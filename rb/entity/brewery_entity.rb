...
class BreweryEntity
  ...
  def make
    opts = @_entopts.dup
    BreweryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenBreweryDbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Brewery, Hash] the current Brewery data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OpenBreweryDbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Brewery fields)
  def match_get
  ...
  #
  # @param reqmatch [BreweryLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Brewery.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Brewery, Hash] the loaded Brewery; raises OpenBreweryDbError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = OpenBreweryDbHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BreweryListMatch, Hash, nil] match filter (any subset of
  #   Brewery fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Brewery>, Array] the matching Brewery items; raises OpenBreweryDbError on failure

...
module OpenBreweryDbUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenBreweryDbResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenBreweryDbResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenBreweryDbResponse.new(fetched)
    else

// Command OpenBreweryDb-cli is a CLI + REPL for the OpenBreweryDb SDK,
...
const prompt = "open-brewery-db"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "brewery"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPEN_BREWERY_DB_APIKEY (api key), OPEN_BREWERY_DB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPEN_BREWERY_DB_APIKEY carries the API key and
	// OPEN_BREWERY_DB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPEN_BREWERY_DB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPEN_BREWERY_DB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenBreweryDbSDK(opts)

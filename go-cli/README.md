...
boru-driven command-line client **and** interactive REPL for the OpenBreweryDb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-brewery-db-cli)
make build

# 2. See usage (words, entities, env vars)
./open-brewery-db-cli --help

# 3. Provide credentials once, via the environment
export OPEN_BREWERY_DB_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./open-brewery-db-cli list brewery
./open-brewery-db-cli load 1 brewery            # {id:1} shorthand
./open-brewery-db-cli load '{id:1}' brewery       # explicit match map

# 5. Override the API base URL for a single call
OPEN_BREWERY_DB_BASE=https://api.example.com ./open-brewery-db-cli list brewery

# 6. No arguments -> interactive REPL
./open-brewery-db-cli
open-brewery-db> list brewery
open-brewery-db> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-brewery-db-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-brewery-db-cli list brewery
   ```
...
```sh
./open-brewery-db-cli load 1 brewery          # scalar shorthand for {id:1}
./open-brewery-db-cli load '{id:1}' brewery     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `open-brewery-db>`). Each line is
evaluated as its own boru expression:

```text
$ ./open-brewery-db-cli
open-brewery-db> list brewery
open-brewery-db> /help
open-brewery-db> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `brewery`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-brewery-db-cli`. |
...
(`./open-brewery-db-cli <expr>`) and interactively (the REPL), and expressions compose the

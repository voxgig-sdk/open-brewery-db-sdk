...
(0, node_test_1.describe)('BreweryDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.OpenBreweryDbSDK({
    ...
    (0, node_test_1.test)('direct-load-brewery', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-brewery', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-brewery', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-brewery', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.OPEN_BREWERY_DB_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.OpenBreweryDbSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['OPEN_BREWERY_DB_TEST_BREWERY_ENTID'];
    ...
    const client = new __1.OpenBreweryDbSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BreweryDirect.test.js.map
...
describe('BreweryDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new OpenBreweryDbSDK({
  ...
  test('direct-load-brewery', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-brewery', setup.live)) return
  ...
  test('direct-list-brewery', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-brewery', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPEN_BREWERY_DB_TEST_BREWERY_ENTID']

...
//     (\`new OpenBreweryDbSDK(...)\` / \`OpenBreweryDbSDK.test(...)\` become
//     \`OpenBreweryDbSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenBreweryDbSDK'
...
const TEST_SEED = {"entity":{"brewery":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenBreweryDbSDK(...)\` and any
// \`OpenBreweryDbSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenBreweryDbSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenBreweryDbSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

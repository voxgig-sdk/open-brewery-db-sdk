...
class TestBreweryEntity:
    ...
    def test_should_stream(self):
        ...
        base = OpenBreweryDbSDK.test(seed, None)
        seen = list(base.Brewery(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from openbrewerydb_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = OpenBreweryDbSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Brewery(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OPEN_BREWERY_DB_TEST_BREWERY_ENTID JSON to run live")
        client = setup["client"]
        ...
        brewery_ref01_list_result = brewery_ref01_ent.list(brewery_ref01_match, None)
        ...
        brewery_ref01_data_dt0_loaded = brewery_ref01_ent.load(brewery_ref01_match_dt0, None)
        brewery_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(brewery_ref01_data_dt0_loaded))
...
def _brewery_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("OPEN_BREWERY_DB_TEST_BREWERY_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("OPEN_BREWERY_DB_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("OPEN_BREWERY_DB_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("OPEN_BREWERY_DB_TEST_EXPLAIN") == "TRUE",
        "live": _live,

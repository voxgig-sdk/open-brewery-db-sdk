# OpenBreweryDb SDK test runner

from __future__ import annotations
import os
import json

from openbrewerydb_sdk.utility.voxgig_struct import voxgig_struct as vs


class OpenBreweryDbTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenBreweryDbTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenBreweryDbTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenBreweryDbTestRunner.getenv("OPEN_BREWERY_DB_TEST_LIVE")
        override = OpenBreweryDbTestRunner.getenv("OPEN_BREWERY_DB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenBreweryDbTestRunner.getenv(key)
        ...
        explain = OpenBreweryDbTestRunner.getenv("OPEN_BREWERY_DB_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPEN_BREWERY_DB_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenBreweryDbTestRunner._test_control is not None:
            return OpenBreweryDbTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenBreweryDbTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenBreweryDbTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenBreweryDbTestRunner.load_test_control()
...
def load_env_local():
    OpenBreweryDbTestRunner.load_env_local()


def env_override(m):
    return OpenBreweryDbTestRunner.env_override(m)


def entity_data(v):
    return OpenBreweryDbTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenBreweryDbTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenBreweryDbTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenBreweryDbTestRunner.load_test_control()


def live_client_options():
    return OpenBreweryDbTestRunner.live_client_options()


def live_delay_ms():
    return OpenBreweryDbTestRunner.live_delay_ms()

...
from openbrewerydb_sdk.openbrewerydb_types import (
    Brewery,
    BreweryLoadMatch,
    BreweryListMatch,
)
...
class BreweryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BreweryEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Brewery:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Brewery().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Brewery]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Brewery().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",

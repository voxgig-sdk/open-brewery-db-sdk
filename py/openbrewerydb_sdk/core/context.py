...
from openbrewerydb_sdk.core.helpers import get_ctx_prop, to_map
...
class OpenBreweryDbContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = OpenBreweryDbControl()
        ...
        if isinstance(sp, OpenBreweryDbSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenBreweryDbResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenBreweryDbResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return OpenBreweryDbError(code, msg, self)

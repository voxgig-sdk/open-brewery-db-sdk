...
local BreweryEntity = {}
BreweryEntity.__index = BreweryEntity


function BreweryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BreweryEntity)
  self._name = "brewery"
  self._client = client
...
function BreweryEntity:get_name()
  return self._name
end


function BreweryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BreweryEntity.new(self._client, opts)
end
...
function BreweryEntity:mark_deleted()
  self._deleted = true
end


function BreweryEntity:deleted()
  return true == self._deleted
end


function BreweryEntity:data_set(args)
  if args ~= nil then
...
function BreweryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BreweryEntity:match_set(args)
  if args ~= nil then
...
function BreweryEntity:match_get()
...
function BreweryEntity:stream(action, args, callopts)
...
function BreweryEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
describe("BreweryDirect", function()
  it("should direct-list-brewery", function()
    local setup = brewery_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-brewery", setup.live and "live" or "unit")
  ...
  it("should direct-load-brewery", function()
    local setup = brewery_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-brewery", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end

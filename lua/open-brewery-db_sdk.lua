...
local OpenBreweryDbSDK = {}
OpenBreweryDbSDK.__index = OpenBreweryDbSDK
...
OpenBreweryDbSDK._make_feature = _make_feature


function OpenBreweryDbSDK.new(options)
  local self = setmetatable({}, OpenBreweryDbSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OpenBreweryDbSDK:options_map()
...
function OpenBreweryDbSDK:get_utility()
  return Utility.copy(self._utility)
end


function OpenBreweryDbSDK:get_root_ctx()
  return self._rootctx
end


function OpenBreweryDbSDK:prepare(fetchargs)
...
function OpenBreweryDbSDK:direct(fetchargs)
...
-- by passing it.
function OpenBreweryDbSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OpenBreweryDbSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OpenBreweryDbSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OpenBreweryDbSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OpenBreweryDbSDK.new(sdkopts)
  sdk.mode = "test"

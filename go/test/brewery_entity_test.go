...
func TestBreweryEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Brewery(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Brewery(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		breweryRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.brewery")))
		...
		breweryRef01DataDt0LoadResult := core.ToMapAny(entityData(breweryRef01DataDt0Loaded))
...
func breweryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPEN_BREWERY_DB_TEST_BREWERY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPEN_BREWERY_DB_TEST_BREWERY_ENTID"])

...
import (
	...
	omni "github.com/voxgig-sdk/open-brewery-db-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenBreweryDbError); is {
				out := map[string]any{
					"name":    "OpenBreweryDbError",
...
func omniCtx(arg any, client *sdk.OpenBreweryDbSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenBreweryDbSDK); is {
			client = live

...
module OpenBreweryDbOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if OpenBreweryDbOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = OpenBreweryDbOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| OpenBreweryDbOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || OpenBreweryDbOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = OpenBreweryDbSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = OpenBreweryDbError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { OpenBreweryDbOmni.errify(err) }

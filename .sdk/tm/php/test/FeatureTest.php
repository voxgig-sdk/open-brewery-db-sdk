...
// OpenBreweryDb SDK feature test
//
...
class FtCtrl extends OpenBreweryDbControl
{
...
class FtClient
{
    ...
    private OpenBreweryDbUtility $utility;

    public function __construct(OpenBreweryDbUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenBreweryDbUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenBreweryDbContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenBreweryDbContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenBreweryDbContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenBreweryDbConfig::make_feature($name);
    ...
    private static function buildUrl(OpenBreweryDbSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenBreweryDbError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenBreweryDbResponse($fetched);
        }
    ...
    private function fail(OpenBreweryDbContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenBreweryDbContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

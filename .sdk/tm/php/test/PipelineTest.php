...
// OpenBreweryDb SDK pipeline test
//
...
{
    private static function utility(): OpenBreweryDbUtility
    {
        return new OpenBreweryDbUtility();
    }
    ...
    private static function ctx(array $over = []): OpenBreweryDbContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenBreweryDbContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenBreweryDbOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenBreweryDbError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenBreweryDbOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenBreweryDbMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenBreweryDbOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenBreweryDbMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenBreweryDbOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenBreweryDbMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenBreweryDbSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenBreweryDbMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenBreweryDbResponse([]),
            'result' => new OpenBreweryDbResult([]),
        ]);
        [, $err] = OpenBreweryDbMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenBreweryDbSpec([]),
            'response' => null,
            'result' => new OpenBreweryDbResult([]),
        ]);
        [, $err] = OpenBreweryDbMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenBreweryDbSpec(['step' => 's']),
            'response' => new OpenBreweryDbResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenBreweryDbResult([]),
        ]);
        [, $err] = OpenBreweryDbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenBreweryDbSpec(['step' => 's']),
            'response' => new OpenBreweryDbResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenBreweryDbResult([]),
        ]);
        [, $err] = OpenBreweryDbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenBreweryDbSpec(['step' => 's']),
            'response' => new OpenBreweryDbResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenBreweryDbResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenBreweryDbMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenBreweryDbResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenBreweryDbSpec([]),
            'response' => new OpenBreweryDbResponse([]),
            'result' => new OpenBreweryDbResult([]),
        ...
        [$got, $err] = OpenBreweryDbMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenBreweryDbResult([])]);
        [, $err] = OpenBreweryDbMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenBreweryDbSpec([]), 'result' => null]);
        [, $err] = OpenBreweryDbMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenBreweryDbSpec(['step' => 's']),
            'result' => new OpenBreweryDbResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenBreweryDbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenBreweryDbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenBreweryDbSpec(['step' => 's']),
            'result' => new OpenBreweryDbResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenBreweryDbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenBreweryDbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenBreweryDbResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenBreweryDbSpec([]),
            'result' => new OpenBreweryDbResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenBreweryDbMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenBreweryDbMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenBreweryDbError('boom', 'boom');
        $utility->fetcher = function (OpenBreweryDbContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenBreweryDbSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenBreweryDbMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenBreweryDbContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenBreweryDbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenBreweryDbResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenBreweryDbMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenBreweryDbContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenBreweryDbSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenBreweryDbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenBreweryDbResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenBreweryDbSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenBreweryDbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenBreweryDbMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenBreweryDbMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenBreweryDbResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenBreweryDbDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenBreweryDbResult(['ok' => false])]);
        $this->expectException(OpenBreweryDbError::class);
        OpenBreweryDbDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenBreweryDbResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenBreweryDbMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenBreweryDbMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenBreweryDbBaseFeature();
        $b = new OpenBreweryDbBaseFeature();
        OpenBreweryDbFeatureAdd::call($ctx, $a);
        OpenBreweryDbFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenBreweryDbBaseFeature
    {
        $f = new OpenBreweryDbBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenBreweryDbFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenBreweryDbContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenBreweryDbSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenBreweryDbPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenBreweryDbPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenBreweryDbResponse(['headers' => null]),
            'result' => new OpenBreweryDbResult([]),
        ]);
        OpenBreweryDbResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenBreweryDbResultBody::call($ctx);

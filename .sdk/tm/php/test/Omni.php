...
//    only - a typed OpenBreweryDbContext there would make every ctx
...
final class OpenBreweryDbOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OpenBreweryDbContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OpenBreweryDbSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OpenBreweryDbResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OpenBreweryDbError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OpenBreweryDbResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OpenBreweryDbError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OpenBreweryDbError(is_string($code) ? $code : '', $msg);
    }

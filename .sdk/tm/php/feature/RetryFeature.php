...
// OpenBreweryDb SDK retry feature
...
class OpenBreweryDbRetryFeature extends OpenBreweryDbBaseFeature
{
    ...
    public function init(OpenBreweryDbContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenBreweryDbContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenBreweryDbContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenBreweryDbContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

...
[MCP](https://modelcontextprotocol.io) server exposing the OpenBreweryDb SDK as
two agent tools — `open-brewery-db_list` and `open-brewery-db_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-brewery-db-mcp)
make build

# 2. Provide credentials via the environment
export OPEN_BREWERY_DB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user open-brewery-db \
  -- /absolute/path/to/open-brewery-db-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./open-brewery-db-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// open-brewery-db_list: first page of records
{ "entity": "brewery" }
{ "entity": "brewery", "query": { } }

// open-brewery-db_load: one record by id
{ "entity": "brewery", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-brewery-db-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user open-brewery-db \
     -- "$PWD"/dist/*/open-brewery-db-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `open-brewery-db_list` and `open-brewery-db_load` tools now appear
   in new sessions. Ask the agent to *"list brewery using open-brewery-db"*
   and it calls `open-brewery-db_list` with `{"entity":"brewery"}`.
...
```sh
export OPEN_BREWERY_DB_APIKEY=sk_live_xxx            # API key
export OPEN_BREWERY_DB_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `open-brewery-db_list` tool
...
### Call the `open-brewery-db_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `open-brewery-db_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `open-brewery-db_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPEN_BREWERY_DB_APIKEY` | API key sent with every request. |
| `OPEN_BREWERY_DB_BASE` | Optional override of the API base URL. |
...
```sh
./open-brewery-db-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"open-brewery-db_load","arguments":{"entity":"brewery","query":{"id":1}}}}'
```
